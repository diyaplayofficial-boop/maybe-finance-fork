...
module LedgerLeaf
  class Application < Rails::Application
    ...
    config.i18n.available_locales = [:en, :"zh-TW", :pt, :es, :ko, :ja]
    ...
    # Skip encryption configuration entirely for self-hosted deployments to avoid credentials issues
    unless ENV["RAILS_SKIP_CREDENTIALS"] == "true"
      begin
        if Rails.application.credentials.active_record_encryption.present?
          config.active_record.encryption = Rails.application.credentials.active_record_encryption
        end
      rescue ActiveSupport::MessageEncryptor::InvalidMessage, LoadError
        # Skip encryption configuration if credentials cannot be decrypted or loaded
        Rails.logger&.warn "Skipping ActiveRecord encryption due to credentials issue"
      end
    end
